...
It is available in R, Python and Julia (to name a few) and can be used to write efficient, clean and safe code.
...
```
`r if (if.beamer) "-"` A language is considered functional if it directs or forces you to create a solution that does not rely on state.
...
`r if (if.latex) "This is a confusing concept, but you've probably already used and encountered this without realising."`
...
$$
```{block include=if.latex}
This problem could be done using some subsetting like `x[as.character(nchar(....))]` but it would very quickly get messy, we could also make multiple variables to do this imperatively.
...
### Anonymous functions
`r if (if.beamer) "-"` An anonymous function is a function definition that is not bound to any identifier.
`r if (if.beamer) "-"` Anonymous functions are also known as _lambdas_ (especially in Python) and _closures_.
